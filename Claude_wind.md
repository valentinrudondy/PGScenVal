...
(the physics paper), `SUPERSEDED_WIND.md` (Stage A wind / Stage C).
...
**Status: the wind half of Stage A and Stage C are SUPERSEDED but RETAINED, not deleted, pending
René's sign-off** (it amends his "core deliverable"). See
`experiments/stage_a_joint_load_wind/SUPERSEDED_WIND.md`. Stage C was never built; its primitive
docstring is flagged unused.
...
- **Stage A (superseded wind half):** `experiments/stage_a_joint_load_wind/` +
  `SUPERSEDED_WIND.md`.
