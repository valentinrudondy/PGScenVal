        ...
        if hist_dfs is not None:
            ...
            for issue_time, fcsts in hist_dfs['forecast'].groupby(
                ...
                # compute difference
                    ...
                    act_stacked = act_df.stack()
                    fcst_stacked = fcst_df.stack()
                    # Align MultiIndex level names for subtraction
                    fcst_stacked.index = fcst_stacked.index.set_names(
                        act_stacked.index.names)
                    hist_dev_dict[
                        fcst_start_time] = act_stacked - fcst_stacked
            ...
            if not isinstance(self.hist_dev_df.index, pd.MultiIndex):
                self.hist_dev_df.index = pd.MultiIndex.from_tuples(
                    self.hist_dev_df.index,
                    names=['horizon', 'asset'],
                )
